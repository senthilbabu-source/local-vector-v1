...
export async function approveAndPublish(
  ...
  if (current.public_slug) {
    ...
    // Fire-and-forget distribution — never blocks approval flow
    distributeMenu(supabase, menuId, ctx.orgId).catch((err) => {
      Sentry.captureException(err, { tags: { component: 'distribution', sprint: 'dist-1' } });
    });
  }
