...
> and the `tests/e2e/` directory. Last updated: Sprint 68 (2026-02-27).
...
|-------|--------|---------|------------------|
| Unit + Integration | Vitest | `npx vitest run` | **818 passing**, 7 skipped (after `supabase db reset`) |
...
| `src/__tests__/unit/audit-cron-ai-audits.test.ts` | ai_audits INSERT, FK linking, graceful failure, clean scan logging, enum values, return type | 11 | 0 | Sprint 68: ai_audits table write — INSERT before hallucination writes, audit_id FK linking, graceful degradation if audit INSERT fails, clean scan logging (is_hallucination_detected true/false), enum validation (model_provider, prompt_type), auditId in result |
| `src/__tests__/unit/sidebar-nav-items.test.ts` | NAV_ITEMS constant, AI Assistant entry, href, active state, position | 5 | 0 | Sprint 68: Sidebar navigation — AI Assistant entry in NAV_ITEMS, correct href /dashboard/ai-assistant, active=true, positioned after Page Audits and before Settings |
