...
describe('buildCronHealthSummary', () => {
  it('returns 16 jobs even with empty input', () => {
    const result = buildCronHealthSummary([]);
    expect(result.jobs).toHaveLength(16);
    expect(result.jobs.map((j) => j.cronName)).toEqual(['audit', 'sov', 'citation', 'content-audit', 'weekly-digest', 'correction-follow-up', 'benchmarks', 'schema-drift', 'review-sync', 'degradation-check', 'correction-benchmarks', 'ai-shopper', 'competitor-vulnerability', 'monthly-report', 'reddit-monitor', 'community-monitor']);
  });
  ...
  it('CRON_REGISTRY has exactly 16 entries', () => {
    expect(CRON_REGISTRY).toHaveLength(16);
  });
