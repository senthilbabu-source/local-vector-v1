...
// Updated §208: computeVoiceReadinessScore now returns { total } & ScoreBreakdown
// ---------------------------------------------------------------------------
...
describe('computeVoiceReadinessScore', () => {
  it('returns max 100 for perfect state', () => {
    const result = computeVoiceReadinessScore('generated', 'healthy', 1.0, 100);
  ...
  it('gives full llms_txt weight (25) when generated', () => {
    ...
    expect(withGenerated.total - withNotGenerated.total).toBe(VOICE_SCORE_WEIGHTS.llms_txt);
  });

  it('gives partial llms_txt weight when stale', () => {
    const withStale = computeVoiceReadinessScore('stale', 'blocked', 0, 0);
    const withNotGenerated = computeVoiceReadinessScore('not_generated', 'blocked', 0, 0);
    expect(withStale.total).toBeGreaterThan(withNotGenerated.total);
    expect(withStale.total).toBeLessThan(VOICE_SCORE_WEIGHTS.llms_txt);
  });
  ...
  it('gives full crawler_access weight (25) when healthy', () => {
    ...
    expect(withHealthy.total - withBlocked.total).toBe(VOICE_SCORE_WEIGHTS.crawler_access);
  });
  ...
  it('gives partial crawler_access weight when partial', () => {
    ...
    const withBlocked = computeVoiceReadinessScore('not_generated', 'blocked', 0, 0);
    expect(withPartial.total).toBeGreaterThan(withBlocked.total);
    expect(withPartial.total).toBeLessThan(VOICE_SCORE_WEIGHTS.crawler_access);
  });

  it('gives some crawler_access weight when unknown', () => {
    const withUnknown = computeVoiceReadinessScore('not_generated', 'unknown', 0, 0);
    const withBlocked = computeVoiceReadinessScore('not_generated', 'blocked', 0, 0);
    expect(withUnknown.total).toBeGreaterThan(withBlocked.total);
  });
  ...
  it('scales voice_citation score linearly with rate', () => {
    ...
    expect(at100.total).toBe(VOICE_SCORE_WEIGHTS.voice_citation);
    expect(at50.total).toBe(Math.round(0.5 * VOICE_SCORE_WEIGHTS.voice_citation));
  });
  ...
  it('scales content_quality score proportionally', () => {
    ...
    expect(at100.total).toBe(VOICE_SCORE_WEIGHTS.content_quality);
    expect(at50.total).toBe(Math.round(0.5 * VOICE_SCORE_WEIGHTS.content_quality));
  });
  ...
  it('computes realistic mid-range score', () => {
    ...
    const result = computeVoiceReadinessScore('generated', 'partial', 0.4, 60);
